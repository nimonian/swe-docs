...
They might not even realise it <em>is</em> a username and password. It's an example of
...
metadata, including the authentication details.</p>
</div>
...
<p>Let's suppose an api endpoint <code v-pre>api/v1/secrets</code> should be only accessible by
authenticated users. If we send a request to this endpoint with the basic
authorization header, we can retrieve it in the backend.</p>
<p>Here is an example based on express.js:</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// get the auth header, e.g. "Bearer bWlja2V5X206SGV5UGx1dG8hMTIz"</span>
...
<span class="token comment">// now we have access to the password to do what we want, for example</span>
<span class="token comment">// we could hash and save the password in the database:</span>
...
<span class="token function">createUserInDB</span><span class="token punctuation">(</span>email<span class="token punctuation">,</span> hashedPassword<span class="token punctuation">)</span>
...
dropped into any endpoint to parse out the authentication header for us.</p>
<h3 id="checking-the-password" tabindex="-1"><a class="header-anchor" href="#checking-the-password" aria-hidden="true">#</a> Checking the password</h3>
<p>Usually, the password has been saved in the database as a hash. To check if the
...
<a href="/backend/hashing#salting">salted</a>, this is a bit tricky. However, most
libraries will do this for use with some kind of <code v-pre>compare</code> function.</p>
<p>Below is an example of this being implemented in express.js. We pick up from
where the above snippet ends.</p>
<div class="language-javascript line-numbers-mode" data-ext="js"><pre v-pre class="language-javascript"><code><span class="token comment">// parse the provided password from the auth header</span>
<span class="token keyword">const</span> <span class="token punctuation">[</span>email<span class="token punctuation">,</span> password<span class="token punctuation">]</span> <span class="token operator">=</span> decodedCredentials<span class="token punctuation">.</span><span class="token function">split</span><span class="token punctuation">(</span><span class="token string">':'</span><span class="token punctuation">)</span>

<span class="token comment">// suppose we get the hash from our database like this</span>
<span class="token keyword">const</span> savedHash <span class="token operator">=</span> <span class="token function">getHashFromDB</span><span class="token punctuation">(</span>email<span class="token punctuation">)</span>

<span class="token comment">// now use bcrypt to check the password against the hash</span>
<span class="token keyword">const</span> result <span class="token operator">=</span> <span class="token keyword">await</span> bcrypt<span class="token punctuation">.</span><span class="token function">compare</span><span class="token punctuation">(</span>password<span class="token punctuation">,</span> savedHash<span class="token punctuation">)</span>
